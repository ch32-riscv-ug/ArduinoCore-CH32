...
> **追記(2026-08-27)**: どちらにも当たらない第3の理由 —— **upstreamがバイナリを
> 公開しておらず、転送すべき成果物が存在しない**(`R-3 不在`)——
> を[ADR-0014](0014-self-built-tool-distribution.ja.md)で提案しています(`Proposed`)。
> このADRの決定は変更していません。
...
> **追記(2026-08-27)**: upstreamの成果物を転送するのではなく**こちらでコンパイルする**
> 場合は、`mirror-`だと来歴の表示が逆になります。接頭辞を`build-`に分ける案を
> [ADR-0014](0014-self-built-tool-distribution.ja.md)で提案しています(`Proposed`)。
