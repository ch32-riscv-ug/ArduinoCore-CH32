...
| A-9 | **USBPDのハードウェアドライバ(X035/X033)** | `libraries/USBPD/src/usbpd_hw.h`(手書きdefine、取り込み後に生成へ置換)、`USBPD.cpp`(CC検出・BMC・GoodCRC・sink state machine) | CH32X035実機: begin()成立、空きポートで未接続報告、capsなしrequest拒否、end/begin再起動——`pd_selftest` 23 check pass。X033/L103/V003 compile可 | **negotiationはPD電源が無く未検証**(再送なし・自動5V request・取り外し非検出の割り切り込み)。X035以外の5 seriesは155c398取り込み待ち | ⬜ |
| A-10 | **`CH32`シングルトン(restart / resetReason / watchdog)** | `libraries/CH32/src/CH32System.{h,cpp}`、`ch32_registers.h`(PFIC CFGR / RSTSCKR / IWDG)、`generate.py`(`CH32_LSI_HZ`・`CH32_IWDG_BASE`をcore_definesへ)、`tests/sketches/basic/system_selftest/` | CH32V103実機: poweron→REBOOT→software→BITE→**watchdog**の3リセットまたぎでfailures=0。V003/X035/M030 compile可 | ESP32寄せ(R-27)はmaintainer方針だが、**個別API(名前・wdtDisable無し・resetReasonのlatch意味論)は私の提案で未承認**。X033/X035のwdtはF_LSIデータ待ち | ⬜ |
| A-11 | **SystemInitを「任意のRCC状態から初期化」する仕様に**(software/watchdogリセット後のクロック破壊の修正) | `cores/arduino/wiring_time.c`(生成マクロ前に無条件でSYSCLK→HSI+SWS待ち) | CH32V203実機: restart→software、watchdog bite→watchdogまで通過、BITE後も115200で読める。全board sweepで回帰確認 | バグ修正自体は明確だが、**PLLクロック対応(A系未承認・未検証)の一部**。144MHz等の高クロック実機確認はまだ | ⬜ |
