...
| # | 対立 | コア側の言い分 |
|---|---|---|
| **C-1** | 「probe は chip を知らない」 vs 「模型を firmware に埋め込む」 | 転送のたびに host へ取りに行く形は SPI/UART で原理的に不可(H-054)。**器で足りるものは器、足りないものだけ模型**、が折衷案 |
| **C-2** | multi-lane(`dmibridge`)vs 1 target 専有(harness) | コアが要るのは後者。ただし将来 board farm で前者も要る(H-009 / H-155)。**同じ仕様の別プロファイルで排他**なら困らない |
...
| **C-4** | capture 帯域 vs control 応答性 | USB FS は ~1 MB/s。flash の bulk とキャプチャが競合する。H-035 の別チャネル化と、動的な degradation の申告 |
...
| **C-8** | 模型を積む(H-015)vs RAM / flash / PIO 命令メモリ | ビルド時プロファイル(H-057)で逃げられるか |
