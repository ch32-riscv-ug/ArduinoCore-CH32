...
6. **W-6/W-7**: host testとsize計測をCI化し、上記すべてをGitHub Actionsへ載せる(**workflow作成済みで初回実行はall green確認済み**(2026-08-19、ubuntu/macos): [.github/workflows/ci.yml](../.github/workflows/ci.yml)。startup-equivalence(EVTミラーをsparse clone)/generated-sync(`vendor/ch32-device-data.lock.toml`のlocked commitでdevice-dataをcheckout)/compile-matrix(size summary付き)/install-testの4 job。その後**windows-latestをcompile-matrixとinstall-testへ追加し、3 OSでall green確認済み**(2026-08-19)。**W-7のsize回帰gateも実装済み**: 26 SKUのBlinkサイズを[sizes_baseline.json](../tests/compile/sizes_baseline.json)に固定し、compile-matrixが完全一致を検証(toolchain固定下ではビルドは決定的。意図的な変化は同一PRで`check_sizes.py --update`により再生成してレビュー)。host test(W-6の一部)のみ未了)
