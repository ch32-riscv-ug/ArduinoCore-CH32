...
| R-27 | [システム系API: ESP32の流儀に寄せる](system-api-esp32-style.ja.md) | restart/resetReason/watchdog/heap/chipId/flash低レベル/sleep/SPI slave/コンパレーターのESP32(ESP8266)前例と写像。`CH32`シングルトン案、実装可能な順序 | Q-011 | **restart/resetReason/watchdogは実装済み(A-10、実機検証済み)。flash/CMP/sleep/SPI slaveは提案のまま** |
