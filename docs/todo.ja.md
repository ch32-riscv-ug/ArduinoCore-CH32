...
- [ ] `[P1]` **`CH32.resetReason()` の RMVF と UIAPduino bootloader**。bootloader は `RCC_RSTSCKR.PINRSTF` が立っている時だけ
...
- [ ] `[P2]` **`SerialN.setPins()` / `setRoute()`は既に全portにある**——`Serial`
      ...
      曖昧**。呼び名とドキュメントを決める
- [x] **`smoke`のreplayに「直前コマンドの1回再送」を追加**(2026-08-25)。
      WCH-Link bridgeがコマンド行を稀に1つ取りこぼす既知quirkで、
      応答待ちがタイムアウトすると全体FAILになっていた(L103の`system_selftest`が
      リセット直後のRUN取りこぼしで1回落ちた。bannerはクリアに読めていたので
