...
- [ ] `[P1]` `[要判断]` **開発機材のリストアップと、常時つなぐ4台の選定**
      ...
      **coreの構造の軸と、それを埋められる型番**:
      ...
      | af-N(per-pin AF、core未対応) | V205 / X305 / X315 | **全て未所有**。X315は未発売 |
      | Mシリーズ | M007 / M030 / M103 | M030の試作基板のみ |
      | 無線 | V208 | 未所有 |

      **いま効率が良いのはV003とV307**——boardは既に手元にあるので、
      **LinkEを足すだけで2軸埋まる**。なお`usbipd`が覚えているWCH-Linkは**6本**で、
      うち2本(COM20 `F90E8F067DFD`、COM24 `38EF8F06BDC2`)は現在未接続。
      これが空いているなら買わずに配線できる。**要確認**
