...
### WCH-LinkのUARTブリッジの癖(2026-08-22実測)

同じboard・同じimageで通ったり通らなかったりする原因を追ったところ、
ブリッジ側に2つの癖があった。どちらもコマンド規約の設計を決めている。

**1. 送るものがあるときにしか吐かない。**
targetがバナーを止めると、**行の途中で止まる**。`heap_string`ではホストに
`string=ab`まで届いてそこで無音になり、いくら待っても続きが来ない。
バナーを0.5秒ごとに出し続けると流れる。よって規約では**バナーを止めない**
(参照実装の`~/dev`は止めるが、あちらはMCU内蔵のUSB-CDC)。

**2. flashのあと配送が止まることがある。**
uploadの直後にportを開いたまま読むと、バナーが2本だけ来て**36秒間無音**になる。
数分前に同じbuildが通っていても起きる。**portを閉じて開き直すと直る**
(`SET_LINE_CODING`が再送されるため)。`smoke.py`は
「uploadを跨いでportを開いたまま(前のsketchの出力を捨て続けるため) →
uploadが終わったら閉じて開き直す」という2段構えにしてある。

**3. 往復が遅い。** `PING`→`PONG`に約5秒。handshakeは12秒、
1ステップの待ちは最低10秒にしてある。
...
- fixture の事実（2026-09-22、`tests/manual/oep_i2c_trace/`）: **route 2（PC16/PC17 ↔ P4 GPIO52/50）に bus pull-up が無い**。P4 の IDF slave driver は内部 pull-up を有効にせず外付けも無い。X035 の INPUT / emulated open-drain release は線上 0、INPUT_PULLUP で 1。`Wire` が動くのは X035 の AF 出力が high を能動駆動するため。emulated OD を試す時は pull-up を用意する
