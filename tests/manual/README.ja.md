...
| （V003）`--target v003` | 上の `oep_gpio_matrix` / `oep_adc_trace` / `oep_periph_trace --only spi-peer` / `oep_i2c_trace --rw` は `tests/manual/oep_smoke/targets.py` の profile で UIAPduino CH32V003（classic ESP32 probe、E132 配線）にも回る。GPIO 12/12、ADC rail、SPI ≤3 MHz、I2C write/read/repeated START を確認（2026-09-22）。capture が無いので PWM/tone/timing/線上 decode は未 |
